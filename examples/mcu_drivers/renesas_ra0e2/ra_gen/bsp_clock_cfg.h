/* generated configuration header file - do not edit */
#ifndef BSP_CLOCK_CFG_H_
#define BSP_CLOCK_CFG_H_
#define BSP_CFG_CLOCKS_SECURE (0)
#define BSP_CFG_CLOCKS_OVERRIDE (0)
#define BSP_CFG_XTAL_HZ (20000000) /* XTAL 20000000Hz */
#define BSP_CFG_XTAL_DIV (BSP_CLOCKS_SYS_CLOCK_DIV_1) /* XTAL Div /1 */
#define BSP_CFG_HOCO_FREQUENCY (2) /* HOCO 32MHz */
#define BSP_CFG_HOCO_DIV (BSP_CLOCKS_SYS_CLOCK_DIV_1) /* HOCO Div /1 */
#define BSP_CFG_MOCO_DIV (BSP_CLOCKS_SYS_CLOCK_DIV_1) /* MOCO Div /1 */
#define BSP_CFG_CLOCK_SOURCE (0) /* ICLK Src: HOCO */
#define BSP_CFG_TAU_CK00 (TIMER_SOURCE_DIV_32) /* TAU CK00 Div /32 */
#define BSP_CFG_TAU_CK01 (TIMER_SOURCE_DIV_1) /* TAU CK01 Div /1 */
#define BSP_CFG_TAU_CK02 (TIMER_SOURCE_DIV_2) /* TAU CK02 Div /2 */
#define BSP_CFG_TAU_CK03 (TIMER_SOURCE_DIV_256) /* TAU CK03 Div /256 */
#define BSP_CFG_SAU_CK00_DIV (BSP_CLOCKS_SAU_CLOCK_DIV_2) /* SAU CK00 Div /2 */
#define BSP_CFG_SAU_CK01_DIV (BSP_CLOCKS_SAU_CLOCK_DIV_1) /* SAU CK01 Div /1 */
#define BSP_CFG_SAU_CK10_DIV (BSP_CLOCKS_SAU_CLOCK_DIV_2) /* SAU CK10 Div /2 */
#define BSP_CFG_SAU_CK11_DIV (BSP_CLOCKS_SAU_CLOCK_DIV_1) /* SAU CK11 Div /1 */
#define BSP_CFG_CLKOUT_SOURCE (BSP_CLOCKS_CLOCK_DISABLED) /* CLKOUT0 Disabled */
#define BSP_CFG_CLKOUT_DIV (0x0U) /* CLKOUT0 Div /1 */
#define BSP_CFG_CLKOUT1_SOURCE (BSP_CLOCKS_CLOCK_DISABLED) /* CLKOUT1 Disabled */
#define BSP_CFG_CLKOUT1_DIV (0x0U) /* CLKOUT1 Div /1 */
#define BSP_CFG_UTASEL_CLOCK_SOURCE (BSP_CLOCKS_SOURCE_CLOCK_HOCO) /* UTASEL Src: HOCO */
#define BSP_CFG_UARTA0_CLOCK_SOURCE (BSP_CFG_UTASEL_CLOCK_SOURCE) /* UARTA0 Src: UTASEL */
#define BSP_CFG_UARTA1_CLOCK_SOURCE (BSP_CFG_UTASEL_CLOCK_SOURCE) /* UARTA1 Src: UTASEL */
#define BSP_CFG_FSXP_SOURCE (BSP_CLOCKS_SOURCE_CLOCK_SUBCLOCK) /* FSXP Src: SUBCLK */
#define BSP_CFG_TML_FITL0_SOURCE (BSP_CFG_FSXP_SOURCE) /* TML FITL0 Src: FSXP */
#define BSP_CFG_TML_FITL1_SOURCE (BSP_CFG_FSXP_SOURCE) /* TML FITL1 Src: FSXP */
#define BSP_CFG_TML_FITL2_SOURCE (BSP_CLOCKS_CLOCK_DISABLED) /* TML FITL2 Disabled/No BSP Source Clock */
#endif /* BSP_CLOCK_CFG_H_ */
